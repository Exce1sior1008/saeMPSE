...
#'
#' @examples 1
mspe_DRS = function( response , designmatrix , sampling.var ){
  A = fayherriot( response , designmatrix , sampling.var )$estimate
  m = length( response )
  p = ncol( designmatrix )
  trsum1 = sum((A + sampling.var )^(-1))
  trsum2 = sum((A + sampling.var )^(-2))
  B =  sampling.var /( sampling.var + A)
  ...
  for(i in 1:m){
    ...
    g3iA[i] = 2 * m * ( sampling.var [i]^2) * (A +  sampling.var [i])^(-3) / (trsum1^2)
  }
